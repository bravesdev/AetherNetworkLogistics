...
namespace aether
{
    public partial class Home : Form
    {
        ...
        private const string CompilatedVersion = "26.5.30";
        ...
        public Home()
        {
            ...
            toolTipDinamico.SetToolTip(btnBackup, "Fazer backup da lista atual.");
            toolTipDinamico.SetToolTip(btnImportBackup, "Importar backup existente.");
            toolTipDinamico.SetToolTip(btnAetherAI, "Assistente de IA Sora.");
            toolTipDinamico.SetToolTip(btnTheme, "Alterne entre o tema claro e escuro.");
            toolTipDinamico.SetToolTip(btnInfo, "Todas as informações do programa.");
        ...
        private async Task VerificarAtualizacao()
        {
            try
            {
                ...
                if (new Version(dados.versao) > new Version(CompilatedVersion))
                {
                    ...
                    if (Msg.Question(msg))
                    {
                        string updater = Path.Combine(Application.StartupPath, "aetherupdater.exe");
        ...
        private void ConfigurarEventosInterfaceIP()
        {
            ...
            cbIPs.SelectedIndexChanged += (s, e) =>
            {
                ...
                bool desejaAcessar = aether.Msg.Question($"Deseja acessar a interface Web do equipamento ({ipSelecionado})?");
                ...
                if (desejaAcessar)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        aether.Msg.Show($"Erro ao abrir o navegador:\n{ex.Message}");
                    }
                }
            };
        }
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            foreach (var id in seriais)
            {
                if (SerialJaExiste(id))
                {
                    ...
                    if (seriais.Count == 1)
                    {
                        Msg.Show($"O equipamento {id} já foi triado.");
            ...
            using (var telaSelecao = new Selecao())
            {
                if (telaSelecao.ShowDialog() == DialogResult.OK && telaSelecao.Selecionados.Count > 0)
                {
                    ...
                    if (seriaisValidos.Count > 1)
                        Msg.Show($"Sucesso\n{seriaisValidos.Count} itens processados em lote.");
                }
            }
        }
...

        {
            ...
            {
                Msg.Show("Não existe itens na lista para gerar um relatorio.");
            }
            else
            {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (registro != null)
                {
                    ...
                    if (Msg.Question(mensagem))
                    {
                        ...
                        // Opcional: Msg.Show("Registro removido com sucesso.");
                    }
                }
            }
        }
        private void RemoverRegistro(RegistroEquipamento registro)
        {
            try
            {
                ...
                Msg.Show("Equipamento removido com sucesso!");
            }
            catch (Exception ex)
            {
                Msg.Show($"Erro ao remover: {ex.Message}");
            }
        }
        ...
        {
            FormManager.OpenSingleForm<Sora>();
        ...
        private void btnTheme_Click(object sender, EventArgs e)
        {
            ...
            if (ThemeManager.CurrentTheme == ThemeManager.ThemeMode.Light)
            {
                ThemeManager.ApplyThemeWithAnimation(this, ThemeManager.ThemeMode.Dark);
            }
            else
            {
                ThemeManager.ApplyThemeWithAnimation(this, ThemeManager.ThemeMode.Light);
            }
        ...
        {
            FormManager.OpenSingleForm<Infomations>();
        ...
        {
            FormManager.OpenSingleForm<Navegador>();
